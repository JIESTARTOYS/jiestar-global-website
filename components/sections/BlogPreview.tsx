...
export function BlogPreview() {
  const newsPost = getBlogSectionPosts("jiestar-news")[0];
  const releasePost = getBlogSectionPosts("new-releases")[0];
  const guideOrBusinessPost = [
    ...getBlogSectionPosts("build-and-collect"),
    ...getBlogSectionPosts("business-insights"),
  ].sort((a, b) => getBlogContentDate(b).localeCompare(getBlogContentDate(a)))[0];
  const posts = [newsPost, releasePost, guideOrBusinessPost].filter(
            ...
            <Link key={post.slug} href={`/blog/${post.slug}`} prefetch={false} className="grid overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md sm:grid-cols-[0.8fr_1fr] md:grid-cols-1">
              ...
              <div className="p-5">
                <p className="text-xs font-black text-red-600">
                  {BLOG_SECTIONS[getBlogSectionForPost(post) ?? "build-and-collect"].title}
                </p>
                ...
                <p className="mt-4 text-xs font-semibold text-slate-400">
                  {post.eventStartDate ? `Event ${formatBlogDate(post.eventStartDate)}` : formatBlogDate(post.date)}
                  {" · "}
