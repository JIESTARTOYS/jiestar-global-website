...
- `app/layout.tsx` 在根节点加入 `translate="no"`、`suppressHydrationWarning`、`body.notranslate` 和 `meta name="google" content="notranslate"`，降低 Chrome 自动翻译改写 DOM 的概率。
...
6. 如果 Chrome 报 `removeChild` / `NotFoundError` 且刷新后恢复，先检查页面是否被 Chrome 翻译过，并确认 `app/layout.tsx` 的 `notranslate` 标记仍存在。
